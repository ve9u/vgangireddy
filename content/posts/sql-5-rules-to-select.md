...
Database Management System(DBMS) manages the data storage and provides an interface for data scientists to fetch the data from the storage. SQL standards are supported by most DBMS softwares and these standards are capable enough to convert any native language query into a database understandable SQL query. 
